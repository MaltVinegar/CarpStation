...
/obj/item/borg/upgrade/modkit/sharpnel/projectile_prehit(obj/projectile/kinetic/K, atom/target, obj/item/gun/energy/kinetic_accelerator/KA)
	if(isliving(target))
		var/mob/living/L = target
		var/datum/status_effect/stacking/saw_bleed/B = L.has_status_effect(STATUS_EFFECT_SAWBLEED)
		if(!B)
			L.apply_status_effect(STATUS_EFFECT_SAWBLEED, modifier)
		else
			B.add_stacks(modifier)
...
/obj/item/borg/upgrade/modkit/shotgun/modify_projectile(obj/projectile/kinetic/K)
	K.damage /= 2
...
/obj/item/borg/upgrade/modkit/chaser/projectile_strike(obj/projectile/kinetic/K, turf/target_turf, atom/target, obj/item/gun/energy/kinetic_accelerator/KA)
	..()
