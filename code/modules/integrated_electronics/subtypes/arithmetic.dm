...
/obj/item/integrated_circuit/arithmetic/exponent/do_work()
	...
	if(IMPORTEDisnum_safe(A.data) && IMPORTEDisnum_safe(B.data))
...
/obj/item/integrated_circuit/arithmetic/round/do_work()
	...
	var/datum/integrated_io/B = inputs[2]
	if(IMPORTEDisnum_safe(A.data))
		if(IMPORTEDisnum_safe(B.data) && B.data != 0)
...
/obj/item/integrated_circuit/arithmetic/random/do_work()
	...
	if(IMPORTEDisnum_safe(L) && IMPORTEDisnum_safe(H))
		result = rand(L, H)
...
/obj/item/integrated_circuit/arithmetic/modulo/do_work()
	...
	if(IMPORTEDisnum_safe(A) && IMPORTEDisnum_safe(B) && B != 0)
		result = A % B
...
/obj/item/integrated_circuit/arithmetic/max/do_work()
	...
	for(var/k in 1 to inputs.len)
		...
		if(!IMPORTEDisnum_safe(I))
			continue
		if(!IMPORTEDisnum_safe(result) || (!min_comparision && I > result) || (min_comparision && I < result))
			result = I
	if(!IMPORTEDisnum_safe(result))
		result = 0
	set_pin_data(IC_OUTPUT, 1, result)
	push_data()
	activate_pin(2)
