...
/obj/item/integrated_circuit/memory/constant/save_special()
	var/datum/integrated_io/O = outputs[1]
	if(istext(O.data) || IMPORTEDisnum_safe(O.data))
		return O.data

/obj/item/integrated_circuit/memory/constant/load_special(special_data)
	var/datum/integrated_io/O = outputs[1]
	if(istext(special_data) || IMPORTEDisnum_safe(special_data))
		O.data = special_data
...
/obj/item/integrated_circuit/memory/constant/attack_self(mob/user)
	...
	switch(type_to_use)
		...
		if("number")
			...
			if(IMPORTEDisnum_safe(new_data) && user.IsAdvancedToolUser())
				O.data = new_data
				to_chat(user, "<span class='notice'>You set \the [src]'s memory to [O.display_data(O.data)].</span>")
		if("ref")
			accepting_refs = TRUE
