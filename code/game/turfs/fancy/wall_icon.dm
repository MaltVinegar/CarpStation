...
/turf/fancyturf/fancy/wall/proc/update_material(var/update = TRUE)
	icon_base = ""
	icon_base_reinf = ""
...
/turf/fancyturf/fancy/wall/proc/set_material(var/material/newmaterial, var/material/newrmaterial)
	material = newmaterial
	reinf_material = newrmaterial
	update_material()

/turf/fancyturf/fancy/wall/update_icon()
	if(!icon_base)
		return
	...
	var/image/I
	for(var/i = 1 to 4)
		I = image('icons/turf/fancyturf/wall_masks.dmi', "[icon_base][wall_connections[i]]", dir = GLOB.cardinal[i])
	...
	if(icon_base_reinf)
		if(construction_stage != null && construction_stage < 6)
			I = image('icons/turf/fancyturf/wall_masks.dmi', "reinf_construct-[construction_stage]")
			I.color = reinf_color
			overlays += I
		else
			if("[icon_base_reinf]0" in icon_states('icons/turf/fancyturf/wall_masks.dmi'))
				// Directional icon
				for(var/i = 1 to 4)
					I = image('icons/turf/fancyturf/wall_masks.dmi', "[icon_base_reinf][wall_connections[i]]", dir = 1<<(i-1))
					I.color = reinf_color
					overlays += I
			else
				I = image('icons/turf/fancyturf/wall_masks.dmi', icon_base_reinf)
				I.color = reinf_color
				overlays += I
...
/turf/fancyturf/fancy/wall/proc/update_connections(propagate = 0)
	if(!material)
		return
	var/list/dirs = list()
	for(var/turf/fancyturf/fancy/wall/W in trange(1, src) - src)
		if(!W.material)
			continue
		if(propagate)
...
/turf/fancyturf/fancy/wall/proc/can_join_with(var/turf/fancyturf/fancy/wall/W)
