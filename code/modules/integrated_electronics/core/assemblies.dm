...
/obj/item/electronic_assembly
	...
	hud_possible = list(IMPORTEDDIAG_STAT_HUD, IMPORTEDDIAG_BATT_HUD, IMPORTEDDIAG_TRACK_HUD, IMPORTEDDIAG_CIRCUIT_HUD) //diagnostic hud overlays
	max_integrity = 50
	pass_flags = 0
...
/obj/item/electronic_assembly/Topic(href, href_list)
	...
	if(href_list["ghostscan"])
		if((isobserver(usr) && ckeys_allowed_to_scan[usr.ckey]) || isAdminObserver(usr))
...
/obj/item/electronic_assembly/proc/try_add_component(obj/item/integrated_circuit/IC, mob/user)
	...
	investigate_log("had [IC]([IC.type]) inserted by [key_name(user)].", IMPORTEDINVESTIGATE_CIRCUIT)
...
/obj/item/electronic_assembly/proc/try_remove_component(obj/item/integrated_circuit/IC, mob/user, silent)
	...
	add_allowed_scanner(user.ckey)
	investigate_log("had [IC]([IC.type]) removed by [key_name(user)].", IMPORTEDINVESTIGATE_CIRCUIT)
