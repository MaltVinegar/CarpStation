...
/obj/item/integrated_circuit/output/screen/large/do_work()
	...
	if(assembly)
		assembly.investigate_log("displayed \"[html_encode(stuff_to_display)]\" with [type].", IMPORTEDINVESTIGATE_CIRCUIT)
	else
		investigate_log("displayed \"[html_encode(stuff_to_display)]\" as [type].", IMPORTEDINVESTIGATE_CIRCUIT)
...
/obj/item/integrated_circuit/output/light/advanced/update_lighting()
	...
	if(new_color && IMPORTEDisnum_safe(brightness))
		brightness = IMPORTEDCLAMP(brightness, 0, 4)
		light_rgb = new_color
...
/obj/item/integrated_circuit/output/sound/do_work()
	...
	if(!isnull(ID) && !isnull(vol))
		...
		if(!selected_sound)
			return
		vol = IMPORTEDCLAMP(vol ,0 , 100)
		playsound(get_turf(src), selected_sound, vol, freq, -1)
		var/atom/A = get_object()
		A.investigate_log("played a sound ([selected_sound]) as [type].", IMPORTEDINVESTIGATE_CIRCUIT)
...
/obj/item/integrated_circuit/output/text_to_radio/Initialize()
	. = ..()
	radio = new(src)
	radio.frequency = IMPORTEDFREQ_COMMON
...
/obj/item/integrated_circuit/output/text_to_radio/proc/recalculate_channels()
	...
	radio.set_frequency(IMPORTEDFREQ_COMMON) //reset it
