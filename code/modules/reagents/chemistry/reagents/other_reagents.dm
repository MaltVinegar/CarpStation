...
/datum/reagent/mutationtoxin/on_mob_life(mob/living/carbon/H)
	. = TRUE
	if(!istype(H))
		return
...
/datum/reagent/mutationtoxin/jelly/on_mob_life(mob/living/H)
	if(isjellyperson(H))
...
/datum/reagent/mulligan/on_mob_life(mob/living/H)
	..()
	if (!istype(H))
		return
...
/datum/reagent/iron/on_mob_life(mob/living/C)
...
/datum/reagent/stimulum/on_mob_life(mob/living/M)
...
/datum/reagent/carpet/royal/on_mob_life(mob/living/M)
	. = ..()
...
/datum/reagent/growthserum/on_mob_life(mob/living/H)
...
/datum/reagent/yuck/on_mob_life(mob/living/C)
	if(!yuck_cycle)
		if(prob(8))
		...
		if(yuck_cycles % YUCK_PUKE_CYCLES == 0)
			...
			if(iscarbon(C))
				...
				N.vomit(rand(14, 26), stun = yuck_cycles >= YUCK_PUKE_CYCLES * YUCK_PUKES_TO_STUN)
	if(holder)
		return ..()
