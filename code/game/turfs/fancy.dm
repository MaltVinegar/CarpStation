...
/turf/fancyturf/fancy/Entered(atom/A, atom/OL)
	...
	if (isliving(A))
		...
		if(ishuman(M))
			...
			if (bloodDNA)
				...
				var/turf/fancyturf/fancy/from = get_step(H,reverse_direction(H.dir))
				if(istype(from) && from)
...
/turf/fancyturf/fancy/proc/can_build_cable(var/mob/user)
	return 0

/turf/fancyturf/fancy/attackby(var/obj/item/thing, var/mob/user)
