...
/obj/item/integrated_circuit/text/concatenator/do_work()
	...
	if(spamprotection >= max_string_length*1.75 && assembly)
		if(assembly.fingerprintslast)
			...
			investigate_log("A concatenator circuit has greatly exceeded its [max_string_length] character limit with a total of [spamprotection] characters, and has been deleted. Assembly last touched by [assembly.fingerprintslast].", IMPORTEDINVESTIGATE_CIRCUIT)
		else
			...
			investigate_log("A concatenator circuit has greatly exceeded its [max_string_length] character limit with a total of [spamprotection] characters, and has been deleted. No associated key.", IMPORTEDINVESTIGATE_CIRCUIT)
		qdel(assembly)
		return
...
/obj/item/integrated_circuit/text/exploders/do_work()
	...
	if(delimiter == null)
		set_pin_data(IC_OUTPUT, 1, IMPORTEDstring2charlist(strin))
	else
