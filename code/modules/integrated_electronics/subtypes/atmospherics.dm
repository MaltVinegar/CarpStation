...
#define TANK_FAILURE_PRESSURE (IMPORTEDONE_ATMOSPHERE*25)
...
/obj/item/integrated_circuit/atmospherics/pump/proc/update_target(new_amount)
	if(!IMPORTEDisnum_safe(new_amount))
		new_amount = 0
...
/obj/item/integrated_circuit/atmospherics/pump/proc/move_gas(datum/gas_mixture/source_air, datum/gas_mixture/target_air)
	...
	if(source_air.return_temperature() < IMPORTEDTCMB)
		source_air.set_temperature(IMPORTEDTCMB)
...
/obj/item/integrated_circuit/atmospherics/pump/filter/on_data_written()
	...
	target_pressure = IMPORTEDCLAMP(amt, 0, PUMP_MAX_PRESSURE)
...
/obj/item/integrated_circuit/atmospherics/pump/mixer/do_work()
	...
	var/transfer_moles = (get_pin_data(IC_INPUT, 5) / max(1,output_gases.return_pressure())) * PUMP_EFFICIENCY * (source_1_gases.return_pressure() * gas_percentage +  source_2_gases.return_pressure() * (1 - gas_percentage)) * output_gases.return_volume()/ (R_IDEAL_GAS_EQUATION * max(output_gases.return_temperature(),IMPORTEDTCMB))
