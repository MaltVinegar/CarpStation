...
/datum/integrated_io/proc/handle_wire(datum/integrated_io/linked_pin, obj/item/tool, action, mob/living/user)
	if(tool.tool_behaviour == TOOL_MULTITOOL)
		if(!tool.multitool_check_buffer(user, tool))
			return
...
/datum/integrated_io/proc/write_data_to_pin(new_data)
	if(isnull(new_data) || IMPORTEDisnum_safe(new_data) || istext(new_data) || isweakref(new_data))
		data = new_data
	...
	switch(type_to_use)
		if("string")
			...
			if(istext(new_data) && holder.check_interactivity(user) )
				...
				return new_data
		if("number")
			new_data = input("Now type in a number.","[src] number writing", IMPORTEDisnum_safe(default) ? default : null) as null|num
			if(IMPORTEDisnum_safe(new_data) && holder.check_interactivity(user) )
				to_chat(user, "<span class='notice'>You input [new_data] into the pin.</span>")
				return new_data
		if("null")
			if(holder.check_interactivity(user))
...
/datum/integrated_io/activate/ask_for_pin_data(mob/user) // This just pulses the pin.
	holder.investigate_log(" was manually pulsed by [key_name(user)].", IMPORTEDINVESTIGATE_CIRCUIT)
