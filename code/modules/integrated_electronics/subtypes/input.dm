...
/obj/item/integrated_circuit/input/numberpad/ask_for_input(mob/user)
	...
	if(IMPORTEDisnum_safe(new_input) && user.IsAdvancedToolUser())
...
/obj/item/integrated_circuit/input/turfpoint/do_work()
	...
	var/target_x = IMPORTEDCLAMP(get_pin_data(IC_INPUT, 1), 0, world.maxx)
	var/target_y = IMPORTEDCLAMP(get_pin_data(IC_INPUT, 2), 0, world.maxy)
...
/obj/item/integrated_circuit/input/advanced_locator_list/on_data_written()
	...
	if(IMPORTEDisnum_safe(rad))
		rad = IMPORTEDCLAMP(rad, 0, 8)
		radius = rad
...
/obj/item/integrated_circuit/input/advanced_locator_list/do_work()
	...
	if(length(input_list))	//if there is no input don't do anything.
		...
		for(var/item in input_list)
			if(!isnull(item) && !IMPORTEDisnum_safe(item))
				if(istext(item))
...
/obj/item/integrated_circuit/input/signaler
	...
	power_draw_per_use = 40
	cooldown_per_use = 5
	var/frequency = IMPORTEDFREQ_SIGNALER
...
/obj/item/integrated_circuit/input/signaler/on_data_written()
	...
	if(IMPORTEDisnum_safe(new_freq) && new_freq > 0)
		set_frequency(new_freq)
	if(IMPORTEDisnum_safe(new_code))
		code = new_code
...
/obj/item/integrated_circuit/input/tcomm_interceptor
	...
	var/list/freq_blacklist = list(IMPORTEDFREQ_CENTCOM,IMPORTEDFREQ_SYNDICATE,IMPORTEDFREQ_CTF_RED,IMPORTEDFREQ_CTF_BLUE)
