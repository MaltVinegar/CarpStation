...
/turf/open/floor/MouseDrop_T(mob/living/M, mob/living/user)
	if(user.stat || !(user.mobility_flags & MOBILITY_STAND) || !Adjacent(user) || !M.Adjacent(user)|| !iscarbon(M))
		if(issilicon(M))
...
/turf/open/pool/attack_hand(mob/living/user)
	if(user.stat == CONSCIOUS && user.mobility_flags & MOBILITY_STAND && !(user.resting) && Adjacent(user) && user.swimming && filled && next_splash < world.time) //not drained, user alive and close, and user in water.
