...
/atom/Click(location,control,params)
	if(flags_1 & INITIALIZED_1)
		...
		// This is a really fucking retarded way to do this but the easiest
		if(modifiers["right"] && !modifiers["Shift"])
			var/tempmove = usr.next_move
			usr.next_move = usr.altnext_move
			usr.swap_hand()
			SEND_SIGNAL(src, COMSIG_CLICK, location, control, params, usr)
			usr.ClickOn(src, params)
			usr.altnext_move = usr.next_move
			usr.next_move = tempmove
			usr.swap_hand()
