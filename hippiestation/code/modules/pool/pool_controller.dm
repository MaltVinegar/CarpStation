		...
		for(var/X in linkedturfs) //Check for pool-turfs linked to the controller.
			...
			for(var/mob/living/M in W) //Check for mobs in the linked pool-turfs.
				...
				if(drownee.stat == DEAD)
					continue
				if(drownee && drownee.resting && !drownee.internal)
...
/obj/machinery/poolcontroller/proc/CanUpTemp(mob/user)
	if(temperature == WARM && (tempunlocked || issilicon(user) || isAdminObserver(user)) || temperature < WARM)
		return TRUE
	return FALSE

/obj/machinery/poolcontroller/proc/CanDownTemp(mob/user)
	if(temperature == COOL && (tempunlocked || issilicon(user) || isAdminObserver(user)) || temperature > COOL)
		return TRUE
	return FALSE
