...
/turf/fancyturf/var/icon_updates_count = 0

/*
/turf/fancyturf/verb/debugupdate()
	set src in view()
...
/turf/fancyturf/fancy/floor/update_icon(var/update_neighbors)
	icon_updates_count++
		...
		//Check if we're actually going to do anything first
			if (isnull(set_update_icon))
				...
				for(var/step_dir in cardinal)
					var/turf/fancyturf/fancy/floor/T = get_step(src, step_dir)
	...
	if(update_neighbors)
		for(var/turf/fancyturf/fancy/floor/F in trange(1, src))
			if(F == src)
				continue
			F.update_icon()
	update_openspace()
...
/decl/flooring/proc/test_link(var/turf/fancyturf/origin, var/turf/fancyturf/T, var/countercheck = FALSE)
	...
	if (countercheck)
		...
		else if (istype(T, /turf/fancyturf/fancy/floor))
			var/turf/fancyturf/fancy/floor/t = T
...
/turf/fancyturf/fancy/floor/proc/get_damage_overlay(var/cache_key, var/icon_base	)
	if(!flooring_cache[cache_key])
		var/image/I = image(icon = 'icons/turf/fancyturf/damage_overlays.dmi', icon_state = icon_base)
...
/turf/fancyturf/fancy/floor/proc/get_flooring_overlay(var/cache_key, var/icon_base, var/icon_dir = 0, var/external = FALSE)
	if(!flooring_cache[cache_key])
