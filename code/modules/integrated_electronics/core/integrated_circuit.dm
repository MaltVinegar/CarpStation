...
/obj/item/integrated_circuit/proc/check_interactivity(mob/user)
	if(assembly)
		...
		return user.canUseTopic(src, IMPORTEDBE_CLOSE)
...
/obj/item/integrated_circuit/Destroy()
	. = ..()
	IMPORTEDQDEL_LIST(inputs)
	IMPORTEDQDEL_LIST(outputs)
	IMPORTEDQDEL_LIST(activators)
...
/obj/item/integrated_circuit/ui_interact(mob/user)
	...
	if(assembly)
		HTML += "<a href='?src=[IMPORTEDREF(src)];return=1'>Return to Assembly</a><br>"

	HTML += "<a href='?src=[IMPORTEDREF(src)]'>Refresh</a>  |  \
		<a href='?src=[IMPORTEDREF(src)];rename=1'>Rename</a>  |  \
		<a href='?src=[IMPORTEDREF(src)];scan=1'>Copy Ref</a>"

	if(assembly && removable)
		HTML += "  |  <a href='?src=[IMPORTEDREF(assembly)];component=[IMPORTEDREF(src)];remove=1'>Remove</a>"
		...
		for(var/j = 1 to column_width)
			...
			switch(j)
				if(1)
					...
					if(io)
						words += "<b><a href='?src=[IMPORTEDREF(src)];act=wire;pin=[IMPORTEDREF(io)]'>[io.display_pin_type()] [io.name]</a> \
							<a href='?src=[IMPORTEDREF(src)];act=data;pin=[IMPORTEDREF(io)]'>[io.display_data(io.data)]</a></b><br>"
						if(io.linked.len)
							words += "<ul>"
							for(var/k in io.linked)
								var/datum/integrated_io/linked = k
								words += "<li><a href='?src=[IMPORTEDREF(src)];act=unwire;pin=[IMPORTEDREF(io)];link=[IMPORTEDREF(linked)]'>[linked]</a> \
	...
	for(var/activator in activators)
		...
		words += "<b><a href='?src=[IMPORTEDREF(src)];act=wire;pin=[IMPORTEDREF(io)]'>[io]</a> \
			<a href='?src=[IMPORTEDREF(src)];act=data;pin=[IMPORTEDREF(io)]'>[io.data?"\<PULSE OUT\>":"\<PULSE IN\>"]</a></b><br>"
