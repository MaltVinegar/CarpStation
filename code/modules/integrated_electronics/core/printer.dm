...
/obj/item/integrated_circuit_printer/Initialize()
	. = ..()
	AddComponent(/datum/component/material_container, list(/datum/material/iron), MINERAL_MATERIAL_AMOUNT * 100, TRUE, list(/obj/item/stack, /obj/item/integrated_circuit, /obj/item/electronic_assembly))
...
/obj/item/integrated_circuit_printer/proc/print_program(mob/user)
	...
	assembly.investigate_log("was printed by [assembly.creator].", IMPORTEDINVESTIGATE_CIRCUIT)
	cloning = FALSE
...
/obj/item/integrated_circuit_printer/interact(mob/user)
	...
	if(idlock)
		...
		HTML+= "[id.name] | <A href='?src=[IMPORTEDREF(src)];id-lock=TRUE'>Reset</a><br>"
	else
		HTML += "None | Reset<br>"
	...
	if((can_clone && CONFIG_GET(flag/ic_printing)) || debug)
		...
		if(!cloning)
			HTML += " <A href='?src=[IMPORTEDREF(src)];print=load'>Load Program</a> "
		else
			HTML += " Load Program"
		if(!program)
			HTML += " [fast_clone ? "Print" : "Begin Printing"] Assembly"
		else if(cloning)
			HTML += " <A href='?src=[IMPORTEDREF(src)];print=cancel'>Cancel Print</a>"
		else
			HTML += " <A href='?src=[IMPORTEDREF(src)];print=print'>[fast_clone ? "Print" : "Begin Printing"] Assembly</a>"

		HTML += "<br><hr>"
	HTML += "Categories:"
	for(var/category in SScircuit.circuit_fabricator_recipe_list)
		if(category != current_category)
			HTML += " <a href='?src=[IMPORTEDREF(src)];category=[category]'>[category]</a> "
	...
	for(var/path in current_list)
		...
		if(ispath(path, /obj/item/integrated_circuit))
			...
			if((initial(IC.spawn_flags) & IC_SPAWN_RESEARCH) && (!(initial(IC.spawn_flags) & IC_SPAWN_DEFAULT)) && !upgraded)
				can_build = FALSE
		if(can_build)
			HTML += "<a href='?src=[IMPORTEDREF(src)];build=[path]'>[initial(O.name)]</a>: [initial(O.desc)]<br>"
		else
...
/obj/item/integrated_circuit_printer/Topic(href, href_list)
	...
	if(href_list["build"])
		...
		if(istype(built, /obj/item/electronic_assembly))
			...
			E.investigate_log("was printed by [E.creator].", IMPORTEDINVESTIGATE_CIRCUIT)
