...
/obj/projectile/colossus/death_orb
	name = "death orb"
	icon_state = "gumball"
	damage = 20
	speed = 3
...
/obj/projectile/death_orb/proc/orb_explosion(projectile_speed_multiplier)
	for(var/i in 0 to 5)
		var/angle = i * 60
		...
		if(!startloc || !endloc)
			break
		var/obj/projectile/colossus/P = new(startloc)
...
/mob/living/simple_animal/hostile/megafauna/colossus/hard/proc/shoot_death_projectile(turf/marker, set_angle)
	if(!isnum(set_angle) && (!marker || marker == loc))
		return
	var/turf/startloc = get_turf(src)
	var/obj/projectile/P = new /obj/projectile/colossus/death(startloc)
...
/mob/living/simple_animal/hostile/megafauna/colossus/hard/proc/shoot_sphere_projectile(turf/marker, set_angle)
	var/turf/startloc = get_turf(src)
	var/obj/projectile/P = new /obj/projectile/colossus/death_orb(startloc)
	P.preparePixelProjectile(marker, startloc)
	P.firer = src
	if(target)
		P.original = target
...
/mob/living/simple_animal/hostile/megafauna/colossus/hard/proc/shpere_shots()
	for(var/tdir in GLOB.alldirs)
		...
		var/obj/projectile/death_orb/orb = shoot_sphere_projectile(T)
		addtimer(CALLBACK(orb, /obj/projectile/death_orb/proc/orb_explosion, 1), 20)
