...
/turf/fancyturf/fancy/wall/levelupdate()
	for(var/obj/O in src)
		O.hide(TRUE)
		SEND_SIGNAL(O, CONSIG_TURF_LEVELUPDATE, TRUE)

/turf/fancyturf/fancy/wall/New(newloc, materialtype, rmaterialtype)
	if (!damage_overlays)
		damage_overlays = new
		...
		for(var/i = 1; i <= overlayCount; i++)
			var/image/img = image(icon = 'icons/turf/fancyturf/walls.dmi', icon_state = "overlay_damage")
...
/turf/fancyturf/fancy/wall/Initialize(mapload)
	..()
...
/turf/fancyturf/fancy/wall/LateInitialize()
...
/turf/fancyturf/fancy/wall/Destroy()
	STOP_PROCESSING(SSturf, src)
	dismantle_wall(null,null,1)
	. = ..()

/turf/fancyturf/fancy/wall/Process(wait, times_fired)
...
/turf/fancyturf/fancy/wall/proc/projectile_reflection(obj/item/projectile/Proj, var/ischance = FALSE)
	if(Proj.starting)
		var/ricochet_temp_id = rand(1,1000)
		if(!ischance)
			Proj.ricochet_id = ricochet_temp_id
		var/turf/fancyturf/curloc = get_turf(src)
		...
		var/walleast = 0
		var/wallwest = 0
		for (var/turf/fancyturf/fancy/wall/W in range(2, curloc))
			var/turf/fancyturf/tempwall = get_turf(W)
...
/turf/fancyturf/fancy/wall/bullet_act(var/obj/item/projectile/Proj)
...
/turf/fancyturf/fancy/wall/hitby(AM as mob|obj, var/speed=THROWFORCE_SPEED_DIVISOR)
	..()
	if(ismob(AM))
		return
...
/turf/fancyturf/fancy/wall/proc/clear_plants()
...
/turf/fancyturf/fancy/wall/ChangeTurf(var/newtype)
	clear_plants()
	clear_bulletholes()
	..(newtype)

//Appearance
/turf/fancyturf/fancy/wall/examine(mob/user)
	. = ..(user)
...
/turf/fancyturf/fancy/wall/melt()

	if(!can_melt())
		return

	src.ChangeTurf(/turf/fancyturf/fancy/floor/plating)

	var/turf/fancyturf/fancy/floor/F = src
	if(!F)
		return
	F.burn_tile()
...
/turf/fancyturf/fancy/wall/proc/take_damage(dam)
	if(dam)
		damage = max(0, damage + dam)
		update_damage()
	return

/turf/fancyturf/fancy/wall/proc/update_damage()
...
/turf/fancyturf/fancy/wall/fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume)//Doesn't fucking work because walls don't interact with air :(
	burn(exposed_temperature)

/turf/fancyturf/fancy/wall/adjacent_fire_act(turf/fancy/floor/adj_turf, datum/gas_mixture/adj_air, adj_temp, adj_volume)
	burn(adj_temp)
...
/turf/fancyturf/fancy/wall/proc/dismantle_wall(devastated, explode, no_product)
	...
	ChangeTurf(/turf/fancyturf/fancy/floor/plating)

/turf/fancyturf/fancy/wall/ex_act(severity)
	switch(severity)
		if(1)
...
/turf/fancyturf/fancy/wall/proc/can_melt()
	if(material.flags & MATERIAL_UNMELTABLE)
		return 0
	return 1

/turf/fancyturf/fancy/wall/proc/thermitemelt(mob/user)
	if(!can_melt())
		return
...
/turf/fancyturf/fancy/wall/proc/radiate()
...
/turf/fancyturf/fancy/wall/proc/burn(temperature)
	if(material.combustion_effect(src, temperature, 0.7))//it wont return something in any way, this proc is commented and it belongs to plasma material.(see materials.dm:283)
		spawn(2)
			...
			src.ChangeTurf(/turf/fancyturf/fancy/floor)
			for(var/turf/fancyturf/fancy/wall/W in trange(3, src) - src)
