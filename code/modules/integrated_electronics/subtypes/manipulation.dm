...
/obj/item/integrated_circuit/manipulation/weapon_firing/do_work()
	...
	mode = mode1.data
	if(assembly)
		if(IMPORTEDisnum_safe(xo.data))
			xo.data = round(xo.data, 1)
		if(IMPORTEDisnum_safe(yo.data))
			yo.data = round(yo.data, 1)

		var/turf/T = get_turf(assembly)
		var/target_x = IMPORTEDCLAMP(T.x + xo.data, 0, world.maxx)
		var/target_y = IMPORTEDCLAMP(T.y + yo.data, 0, world.maxy)
...
/obj/item/integrated_circuit/manipulation/locomotion/do_work()
	...
	if(T && assembly)
		...
		if(assembly.loc == T) // Check if we're held by someone.  If the loc is the floor, we're not held.
			...
			if(IMPORTEDisnum_safe(wanted_dir.data))
...
/obj/item/integrated_circuit/manipulation/grenade/do_work()
	if(attached_grenade && !attached_grenade.active)
		...
		var/dt
		if(IMPORTEDisnum_safe(detonation_time.data) && detonation_time.data > 0)
			dt = IMPORTEDCLAMP(detonation_time.data, 1, 12)*10
		else
			dt = 15
				...
				if(!check_target(O))
					...
					if(!TR.myseed)
						if(istype(O, /obj/item/seeds/kudzu))
							investigate_log("had Kudzu planted in it by [acting_object] at [IMPORTEDAREACOORD(src)]","kudzu")
...
/obj/item/integrated_circuit/manipulation/grabber/proc/grab(obj/item/AM)
	...
	if(check_target(AM))
		if(contents.len < max_items && AM.w_class <= max_w_class)
			...
			A.investigate_log("picked up ([AM]) with [src].", IMPORTEDINVESTIGATE_CIRCUIT)
			AM.forceMove(src)

/obj/item/integrated_circuit/manipulation/grabber/proc/drop(obj/item/AM, turf/T = drop_location())
	var/atom/A = get_object()
	A.investigate_log("dropped ([AM]) from [src].", IMPORTEDINVESTIGATE_CIRCUIT)
	AM.forceMove(T)
...
/obj/item/integrated_circuit/manipulation/claw/do_work(ord)
	...
	switch(ord)
		if(1)
			mode = IMPORTEDCLAMP(mode, GRAB_PASSIVE, max_grab)
			if(AM)
				if(check_target(AM, exclude_contents = TRUE))
					acting_object.investigate_log("grabbed ([AM]) using [src].", IMPORTEDINVESTIGATE_CIRCUIT)
...
/obj/item/integrated_circuit/manipulation/matman/Initialize()
	var/datum/component/material_container/materials = AddComponent(/datum/component/material_container, list(/datum/material/iron, /datum/material/glass, /datum/material/silver, /datum/material/gold, /datum/material/diamond, /datum/material/plasma, /datum/material/uranium, /datum/material/bananium, /datum/material/titanium, /datum/material/bluespace), 0, FALSE, /obj/item/stack, CALLBACK(src, .proc/is_insertion_ready), CALLBACK(src, .proc/AfterMaterialInsert))
	...
	switch(ord)
		...
		if(2)
			...
			for(var/I in 1 to mtypes.len)
				...
				if(M)
					var/U = IMPORTEDCLAMP(get_pin_data(IC_INPUT, I+2),-100000,100000)
					if(!U)
						continue
					if(!mt) //Invalid input
						if(U>0)
